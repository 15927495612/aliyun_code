...
public:
    ...
    string readBytes(int n)
    {
        ...
        if(nr != n)
        {
            throw logic_error("no enought data");
        }
        return string(buf,n);
    }
...
time_t TimeZone::fromLocalTime(const struct tm& localTim) const
{
    assert(data_ != NULL);
    const Data& data(*data_);

    struct tm tmp = localTim;
    ...
    if(localTim.tm_isdst)
    {
        struct tm tryTm = toLocalTime(seconds - local->gmtOffset);
        if(!tryTm.tm_isdst && tryTm.tm_hour == localTim.tm_hour && tryTm.tm_min == localTim.tm_min)
        {
            seconds -= 3600;
        }
    }
